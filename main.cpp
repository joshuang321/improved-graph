...
const LPWSTR lpszWindowName = L"Improved Graph";
...
int WINAPI wWinMain(HINSTANCE hInstance,
	...
	wcex.hIcon 			= LoadIconW(hInstance, (LPCWSTR)IDI_APP);
	wcex.hCursor 		= LoadCursorW(NULL, (LPCWSTR)IDC_ARROW);
	wcex.lpszMenuName 	= MAKEINTRESOURCEW(IDM_GRAPH);
	wcex.lpszClassName 	= lpszWindowName;
	wcex.hIconSm 		= LoadIconW(hInstance, (LPCWSTR)IDI_APP);
	...
	switch (refGraphFunction.functionType) {
		...
		case IDF_LOG: {
			...
			for (;
				...
				y = -refGraphFunction.coefficient * (log(x) / log(refGraphFunction
					.base));
	...
	switch (uMsg) {
		...
		case WM_INITDIALOG: {
			...
			HWND hComboBox = GetDlgItem(hDlg, IDC_COMBOBOX),
				hCTextExtra = GetDlgItem(hDlg, IDC_CTEXT_EXTRA),
				hEditTextExtra = GetDlgItem(hDlg, IDC_EDITTEXT_EXTRA);

			lpGraphFunction = (GraphFunction *)lParam;
			...
			if (IDF_EXP != lpGraphFunction->functionType &&
				...
				switch (lpGraphFunction->functionType) {

					case IDF_EXP: {

						SetWindowTextW(hCTextExtra,L"Base Exponent: ");
						SetWindowTextW(hEditTextExtra, (PWSTR)std::
							to_wstring(lpGraphFunction->baseExp).c_str());
					}
					break;

					case IDF_LOG: {

						SetWindowTextW(hCTextExtra, L"Base Logarithm: ");
						SetWindowTextW(hEditTextExtra, (PWSTR)std::
							to_wstring(lpGraphFunction->base).c_str());
					}
					break;

					case IDF_POWER: {

						SetWindowTextW(hCTextExtra, L"Exponent: ");
						SetWindowTextW(hEditTextExtra, (PWSTR)std::
							to_string(lpGraphFunction->power).c_str());
					}
					break;
				}
			}
			...
			switch (LOWORD(wParam)) {
				...
				case IDOK: {
					...
					if (0 != nWCharCount) {
						...
						MessageBoxW(hDlg, L"Please do not leave a blank field!",
							L"Change Function",
							MB_OK | MB_ICONERROR);
						break;
					}

					if (IsWindowVisible(hEditTextExtra)) {

						nWCharCount = GetWindowTextLengthW(hEditTextExtra);
						...
						if (0 != nWCharCount) {
							...
							lpszWindowString = (LPWSTR)HeapAlloc(GetProcessHeap(),
								...
								(SIZE_T)(nWCharCount + 1) * 2Ui64);
							if (lpszWindowString &&
								nWCharCount == GetWindowTextW(hEditTextExtra,
									lpszWindowString,
									nWCharCount + 1)) {

								newExtra = std::wcstod(lpszWindowString,
									&lppEndPtr);
								HeapFree(GetProcessHeap(), 0, lpszWindowString);
#if defined (__DEBUG)
								LogDebug(std::to_wstring(newExtra));
#endif
								...
								if ((lpszWindowString + nWCharCount) ==
									...
									lpGraphFunction->power = newExtra;
									lpGraphFunction->base = newExtra;
									lpGraphFunction->value = newExtra;
									lpGraphFunction->baseExp = newExtra;
